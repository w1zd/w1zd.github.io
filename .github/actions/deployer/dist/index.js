...
const utils_1$1 = require("./utils");
const os = __importStar$4(require("os"));
...
function exportVariable(name, val) {
    const convertedVal = utils_1$1.toCommandValue(val);
...
Object.defineProperty(exports, "__esModule", { value: true });
const tr = __importStar$3(require("./toolrunner"));
/**
 * Exec a command.
...
var exec$2 = /*#__PURE__*/Object.freeze({
    __proto__: null
});
...
const Context = __importStar$2(require("./context"));
...
Object.defineProperty(exports, "__esModule", { value: true });
const childProcess = __importStar$1(require("child_process"));
const path$2 = __importStar$1(require("path"));
const util_1 = require("util");
const ioUtil$1 = __importStar$1(require("./io-util"));
...
function mv(source, dest, options = {}) {
    return __awaiter$1(this, void 0, void 0, function* () {
        ...
        yield mkdirP$1(path$2.dirname(dest));
...
function mkdirP(fsPath, maxDepth = 1000, depth = 1) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            switch (err.code) {
                case 'ENOENT': {
                    yield mkdirP(path$1.dirname(fsPath), maxDepth, depth + 1);
                    yield exports.mkdir(fsPath);
                    return;
                }
                default: {
...
var ioUtil = /*#__PURE__*/Object.freeze({
    __proto__: null
});
...
async function run() {
  try {
    const accessToken = undefined("access-token");
    const privateKey = undefined("private-key");
    if (!accessToken) {
      undefined(
    ...
    let deployBranch = undefined("deploy-branch");
    if (!deployBranch) deployBranch = DEFAULT_DEPLOY_BRANCH;
    const deployRepo = undefined("deploy-repo");
    const isSameRepo = !deployRepo || deployRepo === undefined;
    if (isSameRepo && undefined === `refs/heads/${deployBranch}`) {
      console.log(`Triggered by branch used to deploy: ${undefined}.`);
      console.log("Nothing to deploy.");
      return
    }
    const workingDir = undefined("working-dir") || ".";
    const pkgManager = (await undefined(`${workingDir}/yarn.lock`))
      ? "yarn"
      : "npm";
    console.log(`Installing your site's dependencies using ${pkgManager}.`);
    await undefined(`${pkgManager} install`, [], { cwd: workingDir });
    console.log("Finished installing dependencies.");
    let gatsbyArgs = undefined("gatsby-args").split(/\s+/).filter(Boolean);
    ...
    await undefined(`${pkgManager} run build`, gatsbyArgs, { cwd: workingDir });
    console.log("Finished building your site.");
    const cnameExists = await undefined(`${workingDir}/CNAME`);
    if (cnameExists) {
      console.log("Copying CNAME over.");
      await undefined(`${workingDir}/CNAME`, `${workingDir}/public/CNAME`, {
        force: true,
      });
      console.log("Finished copying CNAME.");
    }
    const skipPublish = (undefined("skip-publish") || "false").toUpperCase();
    if (skipPublish === "TRUE") {
    ...
    await undefined(`git init`, [], { cwd: `${workingDir}/public` });
    const gitUserName = undefined("git-config-name") || undefined;
    const gitEmail =
      undefined("git-config-email") ||
      `${undefined}@users.noreply.github.com`;
    await undefined(`git config user.name`, [gitUserName], {
      cwd: `${workingDir}/public`,
    });
    await undefined(`git config user.email`, [gitEmail], {
      cwd: `${workingDir}/public`,
    });
    await undefined(`git add`, ["."], { cwd: `${workingDir}/public` });
    const commitMessageInput =
      undefined("commit-message") ||
      `deployed via Gatsby Publish Action 🎩 for ${undefined}`;
    await undefined(`git commit`, ["-m", commitMessageInput], {
      cwd: `${workingDir}/public`,
    });
    await undefined(`git push`, ["-f", repoURL, `master:${deployBranch}`], {
      cwd: `${workingDir}/public`,
    });
